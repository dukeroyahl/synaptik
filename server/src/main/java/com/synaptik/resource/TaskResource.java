...
import com.synaptik.dto.TaskCreateRequest;
import com.synaptik.dto.TaskUpdateRequest;
import com.synaptik.model.Task;
import com.synaptik.model.TaskStatus;
import com.synaptik.service.TaskService;
import com.synaptik.service.TaskWarriorService;
...
public class TaskResource {
    ...
    public Uni<Response> createTask(@Valid TaskCreateRequest request) {
    ...
    public Uni<Response> updateTask(@PathParam("id") String id, @Valid TaskUpdateRequest request) {
    ...
    @POST
    @Path("/capture")
    @Operation(summary = "Quick capture task using TaskWarrior-style syntax")
    @Consumes(MediaType.TEXT_PLAIN)
    public Uni<Response> quickCapture(String input) {
        Task task = taskWarriorService.parseTaskWarriorInput(input);
        return taskService.createTask(task)
            .onItem().transform(createdTask -> Response.status(Response.Status.CREATED).entity(createdTask).build());
    }
    ...
    @GET
    @Path("/search")
    @Operation(summary = "Search tasks by text")
    public Uni<List<Task>> searchTasks(@QueryParam("q") String query) {
        // This would need to be implemented in the repository
        // For now, return all tasks (placeholder)
        return taskService.getAllTasks();
    }
}