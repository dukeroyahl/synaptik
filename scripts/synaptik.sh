...
start_dev() {
    ...
    echo_status "$BLUE" "$INFO" "Starting all services..."
...
check_status() {
    ...
    if [ -f "$PID_FILE" ]; then
        ...
        if ps -p "$SYNAPTIK_PID" > /dev/null; then
            echo_status "$GREEN" "$CHECK" "Synaptik is running (PID: $SYNAPTIK_PID)"
        else
            echo_status "$YELLOW" "$WARNING" "Synaptik is not running (stale PID file)"
            rm "$PID_FILE"
        fi
    else
        echo_status "$BLUE" "$INFO" "Synaptik is not running (no PID file)"
    fi
...
build_production() {
    ...
    echo_status "$BLUE" "$INFO" "Building frontend..."
    npm run build
    
    echo_status "$BLUE" "$INFO" "Building backend..."
